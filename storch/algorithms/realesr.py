    ...
    def forward(self, image):  # noqa: PLR0915, D102
        ...
        if np.random.rand() < self.config.sinc_prob:
            ...
            kernel = random_mixed_kernels(
                self.config.kernel_list,
                self.config.kernel_probs,
                self.config.blur_kernel_size,
                self.config.blur_sigma,
                self.config.blur_sigma,
                [-np.pi, np.pi],
                self.config.betag_range,
                self.config.betap_range,
                None,
            )
        ...
        if np.random.rand() < self.config.second_blur_prob:
            ...
            if np.random.rand() < self.config.sinc_prob2:
                ...
                kernel = random_mixed_kernels(
                    self.config.kernel_list2,
                    self.config.kernel_probs2,
                    self.config.blur_kernel_size2,
                    self.config.blur_sigma2,
                    self.config.blur_sigma2,
                    [-np.pi, np.pi],
                    self.config.betag_range2,
                    self.config.betap_range2,
                    None,
                )
            pad_size = (21 - kernel_size) // 2
            kernel = np.pad(kernel, ((pad_size, pad_size), (pad_size, pad_size)))
        ...
        if np.random.rand() < 0.5:  # noqa: PLR2004
