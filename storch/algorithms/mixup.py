...
def mixup(
    ...
    Arguments:
        images: torch.Tensor
            Tensor of images to apply mixup to.
        targets: torch.Tensor
            Tensor of targets.
        alpha: float (default: 1.0)
            Parameter for sampling random numbers from the Beta distribution.
        p: float (default: 1.0)
            Defaults to 1.0, because mixup is originally always applied.
            Probability to apply mixup to images.
...
def mixed_cross_entropy_loss(logits, targets_a, targets_b, lambdas):
    ...
    Arguments:
        ...
        targets_b: torch.Tensor
            targets returned by mixup()
        lambdas: torch.Tensor
            lambdas returned by mixup()
