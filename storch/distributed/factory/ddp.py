...
class DDPModuleCheckpointInterface(CheckpointInterfaceBase):
    """Interface for checkpointing models wrapped with DDP.

    This class will be constructed automatically by `distributed.DistributedHelper.prepare_for_checkpointing`.

    DDP models holds the original model at `.module` attribute. Getting and setting the state_dict can be achieved by
    accessing this attr. If this is not done, all of the parameters' key will contain `module.` prefix, and we will
    have to erase this prefix when loading parameters to non-parallelized models. This interface provides methods
...
class DistributedDataParallelFactory(ParallelFactoryBase):
    ...
    Factory class that provides methods to wrap the input model with the pytorch native DDP class and creates
    interfaces for checkpointing. This class is only used inside `distributed.DistributedHelper`, and users should not
    be seeing this class from code.
    """

    def wrap_module(self, module: nn.Module, device_ids: list | None = None, **kwargs) -> DDP:
