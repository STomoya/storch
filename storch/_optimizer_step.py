...
def _assert_scaler(maybe_scaler: None|GradScaler):
    assert maybe_scaler is None or isinstance(maybe_scaler, GradScaler), f'"scaler" must be None or an instance of GradScaler.'
...
def simple_optimizer_step(
    ...
    *,
    ...
    This implementation requires to use the GradScaler.enabled attribute for switching
    whether to use gradient scaling/unscaling.


    Args:
        loss (torch.Tensor): loss to backpropagate.
        optimizer (optim.Optimizer): optimizer.
        scaler (GradScaler): Optional GradScaler object. whether to use gradient scaling
            should be configured via "enabled" option in GradScaler.
...
def optimizer_step(
    ...
    *,
    ...
    """optimization step which supports gradient scaling for AMP.
    This is a more general implementation compared to simple_optimizer_step,
    which does not require a GradScaler object.
...
class simple_optimizer_step_with_gradient_accumulation:
    ...
    def __call__(self,
        ...
        *,
        ...
        Args:
            loss (torch.Tensor): loss to backpropagate.
            optimizer (optim.Optimizer): optimizer.
            scaler (GradScaler): Optional GradScaler object.
                If specified, uses it to scale the loss and call .step(). Default: None.
            scheduler (_LRScheduler, optional): learning rate scheduler. should only be specified when calling .step()
                on every batch. Default: None.
            module (nn.Module, optional): The module to optimize. Used for gradient clipping. Default: None.
...
class optimizer_step_with_gradient_accumulation:
    """"torchops.optimizer_step" supporting gradient accumulation.
    This is a more general implementation compared to simple_optimizer_step_with_gradient_accumulation,
    which does not always require a GradScaler object.
