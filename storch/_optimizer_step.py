...
def grad_nan_to_num_(
    input: nn.Module | list[torch.Tensor], nan: float = 0.0, posinf: float = 1e5, neginf: float = 1e-5
) -> None:
...
def _assert_require_module(maybe_module: None | nn.Module):
    assert maybe_module is not None, '"module" is required for "clip_grad_norm" or "grad_nan_to_num" option.'
...
def get_optimizer_step(
    ...
    no_sync_context: Callable | None = None,
    module: nn.Module = None,
    post_backward_hooks: list[Callable] | None = None,
    post_optim_step_hooks: list[Callable] | None = None,
) -> Callable:
    ...
    if post_backward_hooks is None:
        post_backward_hooks = []
    if module is not None and no_sync_context is None:
        no_sync_context = getattr(module, 'no_sync', None)

    func = OptimizerStep(gradient_accumulation_steps, num_iters_per_epoch, no_sync_context)

    if len(post_backward_hooks) > 0:
        assert all(callable(hook) for hook in post_backward_hooks), 'all hooks must be callable.'
        func.register_hooks('backward', *post_backward_hooks)
    if len(post_optim_step_hooks) > 0:
        assert all(callable(hook) for hook in post_optim_step_hooks), 'all hooks must be callable.'
...
class OptimizerStep:
    ...
    def __init__(
        ...
        self.num_iters_per_epoch = (
            num_iters_per_epoch if isinstance(num_iters_per_epoch, int) else gradient_accumulation_steps
        )
    ...
    def __call__(
        ...
        *,
        ...
        ----
            ...
            zero_grad (bool, optional): Call .zero_grad() on optimizer before calling .backward() on loss.
                Default: True.
        ...
        if scaler is None:
            self._general_step(
                ...
                max_norm=max_norm,
                grad_nan_to_num=grad_nan_to_num,
            )
        else:
            self._simple_step(
                optimizer=optimizer,
