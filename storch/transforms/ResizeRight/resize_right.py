...
"""
# ruff: noqa:D101, D103, PLR2004, E402
# Keeping code chagnes minimal.

import warnings
from fractions import Fraction
from math import ceil

from storch.transforms.ResizeRight import interp_methods
...
try:
    ...
    warnings.warn('No PyTorch found, will work only with Numpy', stacklevel=1)
    torch = None
...
try:
    ...
    warnings.warn('No Numpy found, will work only with PyTorch', stacklevel=1)
    numpy = None
...
def calc_pad_sz(in_sz, out_sz, field_of_view, projected_grid, scale_factor, dim_by_convs, fw, device):
    if not dim_by_convs:
        ...
        right_pads = (
            (
                (out_sz - fw_arange(num_convs, fw, device) - 1)  # (1)
                // num_convs
            )  # (2)
            * stride  # (3)
            + field_of_view[:, -1]  # (4)
