...
def bivariate_Gaussian(
    kernel_size: int, sig_x: float, sig_y: float, theta: float, grid: np.ndarray = None, isotropic: bool = True
) -> np.ndarray:
...
def random_bivariate_Gaussian(
    kernel_size: int,
    sigma_x_range: tuple[float],
    sigma_y_range: tuple[float],
    rotation_range: tuple[float],
    noise_range: tuple[float] | None = None,
    isotropic: bool = True,
) -> np.ndarray:
...
def random_bivariate_generalized_Gaussian(
    ...
    if np.random.uniform() < 0.5:  # noqa: PLR2004
...
def random_mixed_kernels(
    ...
    kernel_size: int = 21,
    sigma_x_range: tuple[float] = (0.6, 5),
    sigma_y_range: tuple[float] = (0.6, 5),
    rotation_range: tuple[float] = (-np.pi, np.pi),
    betag_range: tuple[float] = (0.5, 8),
    betap_range: tuple[float] = (0.5, 8),
    noise_range: tuple[float] | None = None,
) -> np.ndarray:
...
def add_gaussian_noise(
    image: np.ndarray, sigma: float = 10.0, gray_noise: bool = False, clip: bool = True, rounds: bool = False
) -> np.ndarray:
...
def random_gaussian_noise(
    image: np.ndarray,
    sigma_range: tuple[float] = (0.0, 10.0),
    gray_prob: float = 0.0,
    clip: bool = True,
    rounds: bool = False,
) -> np.ndarray:
...
def generate_poisson_noise(
    image: np.ndarray, scale: float = 1.0, gray_noise: bool = False, cv2_cvtcolor_mode: int = cv2.COLOR_RGB2GRAY
) -> np.ndarray:
...
def add_poisson_noise(
    ...
    clip: bool = True,
    rounds: bool = False,
    cv2_cvtcolor_mode: int = cv2.COLOR_RGB2GRAY,
) -> np.ndarray:
    """Add poisson noise.
...
def random_poisson_noise(
    image: np.ndarray,
    scale_range: tuple[float] = (0.0, 1.0),
    gray_prob: float = 0.0,
    clip: bool = True,
    rounds: bool = True,
    cv2_cvtcolor_mode: int = cv2.COLOR_RGB2GRAY,
