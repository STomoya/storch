...
def psnr(
    input: torch.Tensor, target: torch.Tensor, max_val: float = 1.0, reduction: str = 'elementwise_mean'
) -> torch.Tensor:
...
def ssim(
    ...
    target: torch.Tensor,
    max_val: float = 1.0,
    kernel_size: int | Sequence[int] = (11, 11),
    sigma: float | Sequence[float] = (1.5, 1.5),
    k1: float = 0.01,
    k2: float = 0.03,
    ...
    filtered = F.conv2d(
        torch.cat([input, target, input * input, target * target, input * target]), gaussian_filter, groups=C
    )
